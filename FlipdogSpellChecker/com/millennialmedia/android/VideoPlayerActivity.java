  ...
  implements MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener, MediaPlayer.OnPreparedListener
{
  ...
  Handler transparentHandler = new Handler()
  {
    public void handleMessage(Message paramAnonymousMessage)
    {
      ...
      {
        return;
        if ((VideoPlayerActivity.this.mVideoView != null) && (VideoPlayerActivity.this.mVideoView.isPlaying()) && (VideoPlayerActivity.this.mVideoView.getCurrentPosition() > 0))
        {
          VideoPlayerActivity.this.mVideoView.setBackgroundColor(0);
          VideoPlayerActivity.this.transparentHandler.sendEmptyMessageDelayed(5, 100L);
          return;
        }
        VideoPlayerActivity.this.transparentHandler.sendEmptyMessageDelayed(4, 50L);
        return;
      }
      while ((VideoPlayerActivity.this.mVideoView == null) || (!VideoPlayerActivity.this.mVideoView.isPlaying()) || (VideoPlayerActivity.this.mVideoView.getCurrentPosition() <= 0));
      VideoPlayerActivity.this.blackView.setVisibility(4);
      VideoPlayerActivity.this.progBar.setVisibility(4);
    }
  };
  ...
  {
    this.redirectListenerImpl = new HttpRedirection.RedirectionListenerImpl()
    {
      public JSONObject getAdProperties()
      {
        if (VideoPlayerActivity.this.adProperties != null)
          return VideoPlayerActivity.this.adProperties.getAdProperties();
        return null;
      }

      public OverlaySettings getOverlaySettings()
      {
        String str = VideoPlayerActivity.this.lastOverlayOrientation;
        OverlaySettings localOverlaySettings = null;
        if (str != null)
        {
          localOverlaySettings = new OverlaySettings();
          localOverlaySettings.orientation = VideoPlayerActivity.this.lastOverlayOrientation;
        }
        return localOverlaySettings;
      }

      public boolean isHandlingMMVideo(Uri paramAnonymousUri)
      {
        VideoPlayerActivity.this.runOnUiThread(new Runnable()
        {
          public void run()
          {
            VideoPlayerActivity.this.enableButtons();
          }
        });
        if ((paramAnonymousUri != null) && (VideoPlayerActivity.this.isActionable(paramAnonymousUri)))
        {
          VideoPlayerActivity.this.processVideoPlayerUri(paramAnonymousUri.getHost());
          return true;
        }
        return false;
      }
    };
    this.redirectListenerImpl.creatorAdImplInternalId = this.activity.creatorAdImplInternalId;
  }
  ...
  protected RelativeLayout initLayout()
  {
    ...
    {
      localLayoutParams3.addRule(2, 83756563);
      initBottomBar(localRelativeLayout);
    }
    this.blackView.setLayoutParams(localLayoutParams3);
    ...
    this.progBar.setLayoutParams(localLayoutParams4);
  ...
  protected void logButtonEvent(VideoImage paramVideoImage)
  {
    ...
    for (int i = 0; i < paramVideoImage.eventLoggingUrls.length; i++)
